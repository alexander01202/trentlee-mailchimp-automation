...
def main():
    ...
    try:
        print("🚀 Starting BizBuySell Combined Scraper...")
        print("=" * 50)
        ...
        print(f"📊 Docs scraped in the last 24h: {len(last24)}")
        ...
        print(f"🔍 Filtered out {len(existing)} URLs already in DB. New URLs to process: {len(filtered_urls)}")

        if not filtered_urls:
            print("⚠️ No new URLs to process after filtering against MongoDB.")
            # Still run notifier for last 24h matches
            notif = notify_subscribers([], list_id="7881b0503b")
            print(f"📨 Mailchimp notify: matched_subscribers={notif['matched_subscribers']}, campaign_sent={notif['campaign_sent']}")
            return True
        ...
        print(f"🚀 Processing {len(filtered_urls)} URLs with Selenium (concurrent: {detail_scraper.max_concurrent})...")
        print("⚠️ Note: Selenium provides better handling of JavaScript-rendered content")
        ...
        print(f"   URLs processed (total scraped vs. new): {len(listing_urls)} vs. {len(filtered_urls)}")
        ...
        print(f"🎉 SUCCESS! Scraper completed. Data saved to MongoDB and notifications sent.")
